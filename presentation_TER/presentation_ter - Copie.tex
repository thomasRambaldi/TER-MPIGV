...
\author[Membres]
{      Yemouna Manel CHIKBOUINI,\\ Kévin LEBRETON,\\Thomas RAMBALDI \\
...
\subsection{License}
	\begin{frame}{Introduction}{License}
		...
		Votre exposé devra présenter les différents aspects du projet : l'analyse de départ qui a été faite, avec d'éventuels choix "stratégiques", la conception , à savoir comment vous avez décidé d'atteindre les buts fixés à l'étape précédente, et enfin la réalisation finale.
		Vous êtes libres d'organiser la présentation comme vous voulez : en suivant l'ordre "chronologique" (en gros ce qui est dit ci-dessus), ou en commençant par une démo qui explicite si vous préférez! Ce qu'il faut c'est faire comprendre ce que vous avez fait, et de préférence pas seulement à votre chargé de TER qui le sait déjà! Faire comprendre, ne signifie pas rentrer dans tous les détails techniques (vous n'en aurez pas le temps), mais donner une idée générale, et si vous le souhaitez mettre en avant un point particulier que vous jugez intéressant.


		L’exposé de soutenance devra comporter les éléments suivants :
		– contexte,
		– contributions,
		– synthèse,
		– bilan,
...
\end{document}\documentclass[10pt]{beamer}
\usetheme[
%%% option passed to the outer theme
%    progressstyle=fixedCircCnt,   % fixedCircCnt, movingCircCnt (moving is deault)
  ]{Feather}
